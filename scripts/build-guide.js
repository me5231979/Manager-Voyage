...
<div class="topic"><span class="n">02</span><div><b>Five ideas every good manager relies on</b><span class="tx2">Priorities, clear expectations, psychological safety, purpose, the people work. Then five moments to practice them.</span></div><span class="pg">Pages 5 to 6</span></div>
...
// learning
H.push(`<section class="brk learn"><span class="eyebrow">Keep learning</span><h2>What to take <em>next</em>.</h2><p class="tx2">A short, chosen list, not the whole catalog. Micro modules first. Then one item a month, on the job your results pointed to. Links are live in the PDF; in Oracle Learning you can also search by title.</p>`);
TOP.forEach(t => { const items = L.filter(i => i.topic === t[0]); if(!items.length) return; H.push(`<h3>${esc(t[1])}</h3><ul style="list-style:none;padding:0">` + items.map(i => `<li><span class="k">${kind(i.type)}</span> <a href="${esc(i.url)}">${esc(i.title)}</a><span class="why">${esc(i.why || '')}</span></li>`).join('') + '</ul>'); });
H.push(`</section>
