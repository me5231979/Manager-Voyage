...
try{
  ...
  var PLAN = [
    ...
    { sel:'#why',             key:'why',       label:'Why the rules exist',     mode:'whole' },
    { sel:'#sequence',        key:'sequence',  label:'The sequence',            mode:'whole' },
    { sel:'#offer',           key:'offer',     label:'The conditional offer',   mode:'whole' },
    { sel:'#salary',          key:'salary',    label:'Salary history',          mode:'whole' },
  ...
  var TRACKED = [
    ...
    ['interview','05','A fair interview'], ['yourcall','06','Your call'], ['quiz','07','Quick check'], ['next','08','Next steps']
  ];
  var pages = [], secFirst = {};
  ...
  var store = (function(){
    try{ var t = 'mv-nyh-page-test'; window.localStorage.setItem(t,'1'); window.localStorage.removeItem(t); return window.localStorage; }
    catch(e){ return null; }
  })();
  function writeHash(p){
    try{ window.history.replaceState(null, '', '#p/' + p.key + '/' + p.n); }catch(e){}
    if(store){ try{ store.setItem('mv-nyh-page', p.key + '/' + p.n); }catch(e){} }
  }
  ...
  if(startIdx === null && store){
    try{
      var saved = store.getItem('mv-nyh-page');
